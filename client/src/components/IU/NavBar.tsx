...
export default function NavBar(): JSX.Element {
  ...
  const toggleDarkMode = () => {
    ...
    setIsDarkMode((prevMode) => {
      ...
      theme.href = prevMode ? '/light-mode.css' : '/dark-mode.css';
  ...
  useEffect(() => {
    ...
    const theme = document.getElementById('theme');
    if (theme) {
      theme.href = isDarkMode ? '/dark-mode.css' : '/light-mode.css';
    }
  }, [isDarkMode]);
            ...
            <div className="hidden sm:ml-6 sm:block">
              <div className="flex space-x-4">
                <NavLink to="/" style={{ color: 'white', marginLeft: '-290px' }}>
                  <img className="h-8 w-8 rounded-full" src="/public/fire.jpg" alt="" />
                </NavLink>
              </div>
            </div>
...
//   const { signOutActionHandler } = authHooks();
//   const user = useAppSelector((state) => state.user);
//   const [isDarkMode, setIsDarkMode] = useState(false); // Добавляем состояние для темы
...
//             {/* Добавляем кнопку switchMode справа */}
//             <button
//               id="switchMode"
//               className="relative rounded-full bg-gray-800 p-1 text-gray-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800"
//             >
